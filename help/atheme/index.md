...
Creating a bot:
    ...
    /msg botserv bot add MyBot Is an.awesome.bot MyBot
...
Modifying a bot:
    ...
    /msg botserv bot change MyBot MyNewBot has a.new.name MyNewBot
...
View available information regarding a channel or bot:
    ...
    /msg botserv info #mychannelname
...
Explanation: This will prevent !op, !kick, !flag, and the like from being
used in-channel. You will have to use ChanServ to complete the desired action
...
Add an email address to a channel:
    ...
    /msg chanserv set #mychannelname email pokemon@fanclubs.com
...
Explanation: This stops ChanServ from applying the current access list
automatically. You can manually set ChanServ to apply all roles using:
...
Create custom templates:
    ...
    /msg chanserv template #mychannelname co-founder +* -H
...
Set the group's email:
    ...
    /msg groupserv set !myawesomegroup email awesome@email.tld
...
Add a URL to a group:
    ...
    /msg groupserv set !myawesomegroup URL http://our.awesome.site

# <a name="HelpServ">HelpServ</a>
...
Create a vHost that users can take (oper only):
    ...
    /msg hostserv offer hide.my.connection
...
Accept an offered vhost:
    ...
    /msg hostserv take hide.my.connection

Remove a previously offered vhost (oper only):

    /msg hostserv unoffer *hide.my.connection

manually assign a vhost (oper only):

    /msg hostserv vhost nickname needs.to.hide
...
Note: 0 means the account doesn't exist, 1 means it exists but is not logged
in, 2 means the user is not logged in but is recognized, and 3 means the user
is logged in  
...
Find users registered using an email address:
    ...
    /msg nickserv listmail *@gmail.com
...
Create an account with NickServ:
    ...
    /msg nickserv register mypassword email@domain.tld
...
Give a user back a stolen account:
    ...
    /msg nickserv return stolen_account a.good@email.address
...
Change email:
    ...
    /msg nickserv set email my.new@email.address
...
<strong>
  ...
  performance and user experience. DO NOT ISSUE THESE COMMANDS LIGHTLY.
  Inject is NOT to be used on a live network. It is included here for
  informational purposes <em>ONLY</em>.
</strong>
</div>
...
OperServ has many more commands, that are covered on the [OperServ](operserv.html) Atheme help page. 
